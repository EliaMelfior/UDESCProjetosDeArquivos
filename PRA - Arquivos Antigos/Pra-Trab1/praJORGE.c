...
void printList(list *list){
	...
	printf("\n %s",nodePonteiro->relatorio->nomeLocal);
	nodePonteiro = nodePonteiro->prox;
	}
}
/**
ponteirodoArquivo printListToFile (list *list, ponteirodoArquivo)
abre o arquivo
repetição percorre os nodes da lista gravando apenas os relatórios no arquivo
retorna o ponteiro do arquivo para que a proxima gravação continue de onde parou
...
char NameSuffix[][5] = {"mar", "rio", "lua", "sol", "marte"};
...
/** \ fn
    Ler o Arquivo 
*/
//falta atualizar com o arquivo do jorge
...
void writeToFileByBlocks(FILE *fppp, long int blockSize, long int sizeInMb)
{
	...
	long int numberOfRegistries, rest;
	int numberOfBlocksNeeded, restOfBlocks, counter;
	...
	restOfBlocks = fileSizeInBytes % blockSizeInBytes;
	...
	createFileByRegistryAmmount(fppp, (restOfBlocks/sizeof(RelatorioClimatico)));
	//Falta tratar o resto da divisão entre blockSizeInBytes e o sizeofRelatorioClimatico
